...
import { PlaybackState, VoiceOption, Chapter, Bookmark } from './types';
...
export default function App() {
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    handleReset();
    setIsLoading(true);
    setLoadingMessage('Parsing your book...');
    setError(null);
  ...
  const playSentence = useCallback(async (index: number) => {
    ...
    setError(null);
    try {
      const audioB64 = await generateSpeech(sentences[index], selectedVoice);
  ...
  }, [sentences, selectedVoice, cleanupAudio, playbackRate]);
  ...
  const handlePlay = () => {
    if (playbackState === 'paused' && audioContextRef.current) {
      ...
      setPlaybackState('playing'); // Set state immediately
  ...
  const handleAddBookmark = () => {
    ...
    const newBookmark: Bookmark = {
        ...
        textSnippet: sentences[currentSentenceIndex].substring(0, 40) + '...',
    };
      ...
      <div className="w-full max-w-2xl mx-auto bg-gray-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col">
        <header className="p-6 bg-gray-900/50 border-b border-gray-700">
          <h1 className="text-2xl font-bold text-center text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-indigo-500">
            PDF Audiobook Narrator
          </h1>
          <p className="text-center text-gray-400 mt-1 text-sm">Powered by Gemini</p>
        </header>
        ...
        <main className="flex-grow p-6 md:p-8">
          {!file ? (
            <div className="flex flex-col items-center justify-center space-y-6">
              ...
              <h2 className="text-xl font-semibold">Upload your book to begin</h2>
              <p className="text-gray-400 text-center max-w-md">Select a PDF file and we'll narrate it for you using a high-quality AI voice.</p>
            ...
            <div className="flex flex-col items-center space-y-6">
              ...
              <div className="w-full p-6 bg-gray-900/70 rounded-lg min-h-[150px] flex items-center justify-center text-center">
                  <p className="text-lg text-gray-300 italic transition-opacity duration-300">
                      {sentences.length > 0 ? sentences[currentSentenceIndex] : "Ready to start."}
                  </p>
              </div>
              ...
              <div className="w-full flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <select id="voice-select" value={selectedVoice} onChange={(e) => setSelectedVoice(e.target.value)} disabled={playbackState !== 'stopped'} className="bg-gray-700 text-white rounded-md px-2 py-1 text-sm border-transparent focus:border-indigo-500 focus:ring-indigo-500 disabled:opacity-50">
              ...
              <div className="w-full grid grid-cols-3 gap-2 items-center text-sm text-gray-400">
                <div className="relative group">
                    <button disabled={chapters.length === 0} onClick={() => {}} className="w-full flex items-center justify-center space-x-2 p-2 rounded-lg bg-gray-700/50 hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"><ListIcon className="w-5 h-5" /><span>Chapters</span><ChevronDownIcon className="w-4 h-4" /></button>
                    <div className="absolute bottom-full mb-2 w-full bg-gray-700 rounded-lg shadow-lg max-h-48 overflow-y-auto hidden group-hover:block z-10">
                        {chapters.map(chap => <a key={chap.sentenceIndex} onClick={() => handleJumpToSentence(chap.sentenceIndex)} className="block px-4 py-2 text-xs truncate hover:bg-gray-600 cursor-pointer">{chap.title}</a>)}
                    </div>
                </div>
                 <div className="relative group">
                    <button onClick={() => {}} className="w-full flex items-center justify-center space-x-2 p-2 rounded-lg bg-gray-700/50 hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"><BookmarkIcon className="w-5 h-5" /><span>Bookmarks</span><ChevronDownIcon className="w-4 h-4" /></button>
                     <div className="absolute bottom-full mb-2 w-full bg-gray-700 rounded-lg shadow-lg max-h-48 overflow-y-auto hidden group-hover:block z-10">
                        {bookmarks.length > 0 ? bookmarks.map(bm => (
                            <div key={bm.sentenceIndex} className="flex items-center justify-between px-4 py-2 hover:bg-gray-600">
                                ...
                                <button onClick={() => handleDeleteBookmark(bm.sentenceIndex)} className="text-red-400 hover:text-red-300 text-xs">X</button>
                            </div>
                ...
                <div className="relative group">
                    <button className="w-full flex items-center justify-center space-x-2 p-2 rounded-lg bg-gray-700/50 hover:bg-gray-700 transition-colors"><SpeedIcon className="w-5 h-5" /><span>{playbackRate}x Speed</span><ChevronDownIcon className="w-4 h-4" /></button>
                     <div className="absolute bottom-full mb-2 w-full bg-gray-700 rounded-lg shadow-lg overflow-y-auto hidden group-hover:block z-10">
                         {PLAYBACK_SPEEDS.map(speed => <a key={speed} onClick={() => setPlaybackRate(speed)} className="block px-4 py-2 text-xs text-center hover:bg-gray-600 cursor-pointer">{speed}x</a>)}
                    </div>
                </div>
              </div>

              <div className="flex items-center space-x-6">
                <button onClick={handleAddBookmark} className="p-3 rounded-full bg-gray-700 hover:bg-gray-600 transition-colors" aria-label="Add Bookmark">
                    <BookmarkIcon className="w-6 h-6 text-white" />
                </button>
                {playbackState === 'playing' || playbackState === 'buffering' ? (
                  <button onClick={handlePause} className="p-4 rounded-full bg-gray-700 hover:bg-gray-600 transition-colors" aria-label="Pause">
                    <PauseIcon className="w-8 h-8 text-white" />
                  </button>
                ) : (
                  <button onClick={handlePlay} className="p-4 rounded-full bg-indigo-600 hover:bg-indigo-500 transition-colors" aria-label="Play">
                    {playbackState === 'buffering' ? <LoaderIcon className="w-8 h-8 text-white animate-spin" /> : <PlayIcon className="w-8 h-8 text-white" />}
                  </button>
                )}
                <button onClick={handleStop} className="p-4 rounded-full bg-gray-700 hover:bg-gray-600 transition-colors" aria-label="Stop">
              ...
              <button onClick={handleReset} className="text-sm text-gray-500 hover:text-gray-300 transition-colors mt-4">
                Upload another book
              </button>
            </div>
          )}
